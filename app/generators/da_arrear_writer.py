...
def write_da_arrear_sheet(ws, arrear_result):
    ...
    if district and district != "MUZAFFARPUR":
        ws["I3"] = f"BLOCK NAME - {block_name} ({district})"
    else:
        ws["I3"] = f"BLOCK NAME - {block_name}"
    ...
    ws["F5"] = f"ACCOUNT NO.- {employee.get('bank_account') or ''}"
    ...
    # 6. Configure print layout: fit to one A4 landscape page
    configure_print_layout(ws, last_col_letter='Q', last_row=sig_row)
